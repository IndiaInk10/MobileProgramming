...
public class Self1202 extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.self1202_main_org);
        ...
        btnInsert.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (edtName.getText().toString().equals("")  || edtNumber.getText().toString().equals("")) {
                    Toast.makeText(getApplicationContext(), "데이터를 입력하세요",
                            Toast.LENGTH_SHORT).show();
                } else {
                    sqlDB = myHelper.getReadableDatabase();
                    Cursor cursor;
                    cursor = sqlDB.rawQuery("SELECT * FROM groupTBL WHERE gName = '"
                    ...
                    if (cursor.getCount() == 0) {
                        ...
                        Toast.makeText(getApplicationContext(), "해당하는 이름이 존재합니다..",
                                Toast.LENGTH_SHORT).show();
                    }
                    cursor.close();
                    sqlDB.close();
            ...
            public void onClick(View v) {
                if (edtName.getText().toString().equals("")  || edtNumber.getText().toString().equals("")) {
                    ...
                    cursor = sqlDB.rawQuery("SELECT * FROM groupTBL WHERE gName = '"
                            + edtName.getText().toString() + "';", null);
                    if (cursor.getCount() == 0) {
                        Toast.makeText(getApplicationContext(), "해당하는 이름이 존재하지 않습니다..",
                                Toast.LENGTH_SHORT).show();
                    } else {
                        sqlDB = myHelper.getWritableDatabase();
            ...
            public void onClick(View v) {
                if (edtName.getText().toString().equals("")) {
                    ...
                    cursor = sqlDB.rawQuery("SELECT * FROM groupTBL WHERE gName = '"
                            + edtName.getText().toString() + "';", null);
                    if (cursor.getCount() == 0) {
                        Toast.makeText(getApplicationContext(), "해당하는 이름이 없습니다.",
                                Toast.LENGTH_SHORT).show();
                    } else {
                        sqlDB = myHelper.getWritableDatabase();
